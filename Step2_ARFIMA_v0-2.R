...
dev.new(width=12,height=6)
par(mfrow=c(3,1),mex=0.75)
plot.ts(r.t_facebook_1,ylim=c(-2,2),
...
dev.new(height=6,width=12)
par(mfrow=c(1,2),mex=0.75)
hist(r.t_facebook_1,                                                                           # one outlier
     breaks=seq(-2,5,0.25),
     freq=FALSE,
     col="grey85",ylim=c(0,3),
     main="Residual Histogram")                                                              
z <- seq(-60,60,length=1000)                                      
lines(z,dnorm(z,mean=mean(r.t_facebook_1),sd=sd(r.t_facebook_1)),lty=1,col="red")                   # add theoretical normal density
